...
const runMigrations = () => {
  // Check Items table
  const itemsInfo = db.pragma('table_info(items)');
  ...
  const expensesInfo = db.pragma('table_info(expenses)');
  ...
  // Check Invoices table for status (optional, but good for reporting)
  const invoicesInfo = db.pragma('table_info(invoices)');
  ...
  const invoiceItemsInfo = db.pragma('table_info(invoice_items)');
...
export const initDatabase = () => {
  try {
    ...
    const forceAdd = (table, column, type, defaultValue) => {
      try {
