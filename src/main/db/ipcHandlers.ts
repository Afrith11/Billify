...
export const registerIpcHandlers = () => {
  ...
  ipcMain.handle('add-item', (_, item: any) => {
    try {
  ...
  ipcMain.handle('update-item', (_, item: any) => {
    try {
      const stmt = db.prepare(`
        UPDATE items SET 
  ...
  ipcMain.handle('add-customer', (_, customer: any) => {
    try {
      const stmt = db.prepare(`
  ...
  // Invoices (Day Book)
  ipcMain.handle('get-invoices', (_, filters: any) => {
    try {
  ...
  ipcMain.handle('get-invoice-by-id', (_, id: number) => {
    try {
      return db.prepare(`
  ...
  ipcMain.handle('get-invoice-returns', (_, invoiceId: number) => {
    try {
      return db.prepare(`
  ...
  ipcMain.handle('get-invoice-items', (_, invoiceId: number) => {
    try {
      return db.prepare(`
  ...
  ipcMain.handle('get-next-invoice-number', () => {
    try {
      ...
      const lastInvoice = db.prepare('SELECT invoice_number FROM invoices WHERE invoice_number LIKE ? ORDER BY id DESC LIMIT 1').get(searchPattern) as any;
  ...
  ipcMain.handle('create-invoice', (_, invoice: any) => {
    try {
      const transaction = db.transaction(() => {
        // 1. Insert Invoice
        ...
        return { success: true, invoiceNumber: invoice.invoice_number, id: invoiceId };
      });
      return transaction();
    } catch (error: any) {
  ...
  // Payments
  ipcMain.handle('get-payments', (_, filters: any) => {
    try {
  ...
  ipcMain.handle('add-payment', (_, payment: any) => {
    try {
      const transaction = db.transaction(() => {
        // 1. Insert Payment
        const payStmt = db.prepare(`
  ...
  ipcMain.handle('receive-payment', (_, payment: any) => {
    // Alias for add-payment to match preload
    const events = (ipcMain as any)._events;
    if (events['add-payment']) {
      return events['add-payment'](null, payment);
    }
    return { success: false, message: 'Handler not found' };
  });

  // Returns
  ipcMain.handle('get-sales-returns', (_, filters: any) => {
    try {
      let query = `
  ...
  ipcMain.handle('create-sales-return', (_, data: any) => {
    try {
      const transaction = db.transaction(() => {
        ...
        // 4. Mark invoice as partially/fully returned
  ...
  ipcMain.handle('get-purchase-returns', (_, filters: any) => {
    try {
      let query = `
        SELECT pr.*, e.vendor_name 
        FROM purchase_returns pr 
        JOIN expenses e ON pr.expense_id = e.id
      `;
      const params: any[] = [];
      if (filters?.fromDate && filters?.toDate) {
  ...
  ipcMain.handle('create-purchase-return', (_, data: any) => {
    try {
      const transaction = db.transaction(() => {
        // 1. Create Return Record
        const stmt = db.prepare(`
          ...
          VALUES (?, ?, ?, ?)
        `);
        const info = stmt.run(data.expense_id, data.return_date, data.amount, data.notes);
        
        // 2. Update Expense Status (Optional: Mark as partially returned or similar)
        // For simplicity, we just keep the record.
  ...
  ipcMain.handle('get-dashboard-stats', () => {
    try {
      ...
      const todaySales = (db.prepare("SELECT SUM(net_amount) as total FROM invoices WHERE bill_date = ? AND status != 'Cancelled'").get(today) as any).total || 0;
      const monthlySales = (db.prepare("SELECT SUM(net_amount) as total FROM invoices WHERE bill_date >= ? AND status != 'Cancelled'").get(firstDayOfMonth) as any).total || 0;
      const totalCustomers = (db.prepare('SELECT COUNT(*) as count FROM customers').get() as any).count || 0;
      
      const receivables = (db.prepare(`
        SELECT SUM(CASE WHEN type = 'Debit' THEN amount ELSE -amount END) as balance 
        FROM ledger_entries
      `).get() as any).balance || 0;
      ...
      const totalPayable = (db.prepare("SELECT SUM(amount) as total FROM expenses WHERE status = 'Unpaid'").get() as any).total || 0;
      
      // Real Expenses for Profit calculation (Current Month)
      const monthlyExpenses = (db.prepare("SELECT SUM(amount) as total FROM expenses WHERE expense_date >= ?").get(firstDayOfMonth) as any).total || 0;
  ...
  ipcMain.handle('get-receivables-report', () => {
    try {
      const customers = db.prepare('SELECT id, name, phone, customer_id as code FROM customers').all() as any[];
      const report = customers.map(cust => {
        const balance = (db.prepare(`
          SELECT SUM(CASE WHEN type = 'Debit' THEN amount ELSE -amount END) as balance 
          FROM ledger_entries WHERE customer_id = ?
        `).get(cust.id) as any).balance || 0;
        ...
        for (const debit of debits) {
          ...
          const diffDays = Math.floor((now.getTime() - debitDate.getTime()) / (1000 * 60 * 60 * 24));
  ...
  ipcMain.handle('get-advanced-sales-report', (_, filters: any) => {
    try {
      let query = `
        SELECT 
          i.id as invoice_id,
  ...
  ipcMain.handle('get-receipt-statements', (_, filters: any) => {
    try {
      let query = `
        SELECT 
          p.*, 
  ...
  ipcMain.handle('add-expense', (_, expense: any) => {
    try {
      const stmt = db.prepare(`
  ...
  ipcMain.handle('update-expense-status', (_, { id, status }: any) => {
    try {
  ...
  ipcMain.handle('get-daybook', (_, { from, to }: any) => {
    try {
      // Invoices (Debit)
      ...
      return [...invoices as any[], ...payments as any[], ...expenses as any[], ...returns as any[]]
  ...
  ipcMain.handle('get-ledger-report', (_, customerId: number) => {
    try {
      return db.prepare(`
        SELECT *, 
  ...
  ipcMain.handle('upload-logo', (_, { name, data }: any) => {
    try {
  ...
  ipcMain.handle('check-auth-status', () => {
    try {
      const settings = db.prepare('SELECT auth_enabled FROM settings WHERE id = 1').get() as any;
  ...
  ipcMain.handle('create-admin', (_, { username, password }: any) => {
    try {
  ...
  ipcMain.handle('login', (_, { username, password }: any) => {
    try {
  ...
  ipcMain.handle('change-password', (_, { username, oldPassword, newPassword }: any) => {
    try {
      const user = db.prepare('SELECT * FROM users WHERE username = ?').get(username) as any;
  ...
  ipcMain.handle('toggle-auth', (_, enabled: boolean) => {
    try {
  ...
  ipcMain.handle('add-user', (_, { username, password, role }: any) => {
    try {
  ...
  ipcMain.handle('update-user', (_, { id, username, role, password }: any) => {
    try {
      const normalizedRole = role.toLowerCase();
      if (password) {
        const hash = hashPassword(password);
  ...
  ipcMain.handle('toggle-user-status', (_, { id, isActive }: any) => {
    try {
